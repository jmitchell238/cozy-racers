...
function updateMenuStats() {
  ...
  document.querySelectorAll('.car-chip').forEach(btn => {
    btn.classList.toggle('active', Number(btn.dataset.car) === (save.carIndex | 0));
  });
}
...
function onKeyDown(e) {
  if (e.key === 'Escape' && (state === 'play' || state === 'win')) {
    showMenu();
    return;
  }
  if (state !== 'play') return;
...
function onKeyUp(e) {
  ...
  if (!left && !right) clearSteer();
  else if (left) setSteerX(playerX - 80);
  else if (right) setSteerX(playerX + 80);
}
...
function wireUi() {
  ...
  document.querySelectorAll('.car-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      setCarIndex(Number(btn.dataset.car));
      sfxClick();
      updateMenuStats();
    });
  });
...
/** Reload when idle; defer mid-play so a race isn't interrupted. */
